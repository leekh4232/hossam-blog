...
**실습 파일:** `03-컬렉션,순회,수정/01-array_slice.go`
...
### 실행 예제 다이어그램: 배열과 슬라이스의 값/참조 동작 차이
...
graph TD
    ...
    subgraph modifyArray(arr [3]int)
        C[arr_copy: [1,2,3]]
        C --> D{arr_copy[0] = 99}
        D --> E[arr_copy: [99,2,3]]
    end

    subgraph modifySlice(s []int)
        F[s_header_copy: len=3, cap=3, ptr -> [4,5,6]]
        F --> G{s_header_copy.ptr[0] = 99}
        G --> H[ptr -> [99,5,6]]
    end

    A -- 값 복사 --> C
    B -- 헤더 복사 (내부 배열 공유) --> F
    E -- 함수 종료 --> I[main 함수 arr1: [1,2,3] (변화 없음)]
    H -- 함수 종료 --> J[main 함수 slice1: [99,5,6] (변화 있음)]
```
...
**실습 파일:** `03-컬렉션,순회,수정/02-slice-append-delete.go`
...
### 실행 예제 다이어그램: 슬라이스 `append` 시 용량 초과

```mermaid
graph TD
    subgraph 초기 슬라이스
        A[slice: len=3, cap=3] --> B(내부 배열: [1, 2, 3])
    end

    subgraph append(slice, 4) 호출
        C{용량 부족 판단 (cap=3, 필요한 공간=4)}
        C --> D[새로운 내부 배열 할당 (cap=6)]
        D --> E(기존 요소 복사: [1, 2, 3, _, _, _])
        E --> F(새 요소 추가: [1, 2, 3, 4, _, _])
        F --> G[새 슬라이스 반환 (len=4, cap=6)]
    end

    A -- append 호출 --> C
    G -- 재할당 --> H[slice: len=4, cap=6] --> I(내부 배열: [1, 2, 3, 4, _, _])
```

### 실행 예제 다이어그램: 슬라이스 요소 삭제

```mermaid
graph TD
    subgraph 초기 슬라이스
        A[sliceToDelete: [10, 20, 30, 40, 50]]
    end

    subgraph 인덱스 1 (20) 삭제
        B{sliceToDelete[:1] -> [10]}
        C{sliceToDelete[2:] -> [30, 40, 50]}
        B & C --> D{append([10], [30, 40, 50]...)}
        D --> E[결과: [10, 30, 40, 50]]
    end

    A -- 삭제 연산 --> B
    E -- 재할당 --> F[sliceToDelete: [10, 30, 40, 50]]
```
...
### 실행 예제 다이어그램: 맵(Map)의 주요 연산

```mermaid
graph TD
    subgraph 초기 맵
        A[scores: {"Alice": 90, "Bob": 85}]
    end

    subgraph 값 추가/수정
        B{playerStats["Health"] = 100}
        C{playerStats["Mana"] = 50}
        D{playerStats["Health"] = 90}
        A -- 초기화 --> B
        B --> C
        C --> D
        D --> E[playerStats: {"Health": 90, "Mana": 50}]
    end

    subgraph 값 가져오기 및 존재 확인
        F{mana = playerStats["Mana"]}
        G{hp, ok = playerStats["Health"]}
        H{gold, ok = playerStats["Gold"]}
        E -- 가져오기 --> F
        F --> G
        G --> H
        H --> I[결과: mana=50, hp=90, ok=true, gold=0, ok=false]
    end

    subgraph 값 삭제
        J{delete(playerStats, "Mana")}
        K{delete(playerStats, "NonExistentKey")}
        I -- 삭제 --> J
        J --> K
        K --> L[playerStats: {"Health": 90}]
    end
```
...
**실습 파일:** `03-컬렉션,순회,수정/04-iterate.go`
...
## 7. 실습 5: 최종 프로젝트 - 학생 점수 집계 및 평균 계산

앞서 배운 컬렉션 자료형과 순회 방법을 활용하여 간단한 학생 점수 집계 프로그램을 만들어 본다.

**실습 파일:** `03-컬렉션,순회,수정/05-score-avg.go`
...
func main() {
    ...
    // 7.2 `for-range`를 이용한 맵 순회 및 합계 계산
...
이번 포스팅에서는 Go의 컬렉션 자료형을 Java와 비교하며, 실습을 통해 선언, 수정, 순회, 집계까지의 과정을 단계별로 익혔습니다. 실습 파일을 직접 작성하며 Go의 문법과 특징을 체험해보세요.

다음 포스팅에서는 Go의 함수와 메서드, 그리고 객체지향적 활용법을 다룰 예정입니다.
