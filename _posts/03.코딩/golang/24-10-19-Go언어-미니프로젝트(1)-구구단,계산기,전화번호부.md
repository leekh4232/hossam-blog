...
**실습 파일: `05-미니프로젝트(1)/01-gugudan/main.go`**

아래 코드는 사용자로부터 숫자를 입력받아 유효성을 검사하고, `for` 루프를 이용해 해당 숫자의 구구단을 출력하는 과정을 보여줍니다. 사용자의 입력은 문자열이므로 `strconv.Atoi`를 통해 정수로 변환하며, 변환 실패나 범위 오류가 발생하면 에러 메시지를 출력하고 프로그램을 종료합니다.
...
**실습 파일: `05-미니프로젝트(1)/02-calculator/main.go`**

이 프로그램은 `main` 함수에서 사용자 입력을 받고, `calculate` 함수를 호출하여 실제 계산을 수행합니다. `calculate` 함수는 `switch` 문을 통해 연산자별로 다른 계산을 처리하며, 0으로 나누는 등의 예외 상황이 발생하면 결과와 함께 에러를 반환합니다. `main` 함수는 이 에러를 받아 적절한 메시지를 사용자에게 보여줍니다.

```mermaid
sequenceDiagram
    participant User
    participant main as "main()"
    participant calculate as "calculate()"

    main->>User: 첫 번째 숫자를 입력하세요:
    User->>main: (숫자1 입력)
    main->>User: 연산자를 입력하세요 (+, -, *, /):
    User->>main: (연산자 입력)
    main->>User: 두 번째 숫자를 입력하세요:
    User->>main: (숫자2 입력)

    main->>calculate: calculate(숫자1, 숫자2, 연산자)
    activate calculate
    calculate->>calculate: switch (연산자)
    alt 0으로 나누기 등 에러
        calculate-->>main: (0, error)
    else 정상 계산
        calculate-->>main: (결과, nil)
    end
...
**실습 파일: `05-미니프로젝트(1)/03-phonebook/main.go`**

이 프로그램은 `for` 무한 루프를 통해 사용자에게 지속적으로 메뉴를 보여주고, `switch` 문으로 사용자의 선택에 따라 각기 다른 함수(`addContact`, `viewContact` 등)를 호출합니다. 전역 변수로 선언된 `phoneBook` 맵이 실제 데이터 저장소 역할을 하며, 각 함수는 이 맵의 데이터를 조작하여 연락처를 관리합니다. 사용자가 '5'를 선택하면 `return`을 통해 `main` 함수가 종료되면서 프로그램이 끝납니다.

```mermaid
sequenceDiagram
    participant User as "사용자"
    participant Main as "main()"
    participant Add as "addContact()"
    participant View as "viewContact()"
    participant Delete as "deleteContact()"
    participant List as "listAllContacts()"

    loop 메뉴 반복
        Main->>User: 메뉴 출력
        User->>Main: 선택 입력

        Note over Main: switch 문으로 분기
        ...
        Main->>List: 4번 선택시 호출
        activate List
        List->>List: phoneBook 전체 조회
        List->>User: 모든 연락처 출력
        List-->>Main: 완료
        deactivate List

        Note over Main: 5번 선택시 루프 종료
    end
```
