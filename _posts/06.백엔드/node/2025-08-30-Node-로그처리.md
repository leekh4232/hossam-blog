    ...
    transports: [
        ...
        new winston.transports.Console({
            ...
            format: winston.format.combine(
                ...
                winston.format.simple() // 간단한 형식으로 출력
            )
        }),

        // 2) 파일에 모든 레벨의 로그 저장 (일별 파일 생성)
        new DailyRotateFile({
            filename: 'logs/app-%DATE%.log', // 로그 파일명 형식. %DATE%는 날짜로 자동 치환됨.
...
const logger = winston.createLogger({
    // 로그 레벨 설정
    level: 'debug', // 개발 중에는 모든 레벨의 로그를, 운영 시에는 info나 warn으로 조정
    ...
    transports: [
        ...
        new winston.transports.Console({
            format: winston.format.combine(
                winston.format.colorize({ all: true }), // 로그 레벨에 따라 색상 적용
                winston.format.simple() // 간단한 로그 형식
            )
        }),

        // 2) 모든 로그를 저장할 일별 로그 파일 설정
        new DailyRotateFile({
            filename: 'logs/app-%DATE%.log',    // 파일명 형식
        ...
        // 3) 에러 로그만 별도로 저장할 일별 로그 파일 설정
        new DailyRotateFile({
            level: 'error',                     // error 레벨만 기록
            filename: 'logs/error-%DATE%.log',  // 파일명 형식
            datePattern: 'YYYY-MM-DD',        // 날짜 패턴
            zippedArchive: true,              // 압축 저장 여부
            maxSize: '20m',                   // 파일 최대 크기
            maxFiles: '30d'                   // 보관 기간 (30일)
        })
    ]
});
